...
export function apply(ctx: Context) {
  ...
  ctx.tools.register(
    pyTool({
      ...
      parameters: {
        symbol: { type: "string", required: true, description: "股票代码（A股如600519，美股如AAPL，港股如00700.HK）" },
        periods: { type: "number", description: "返回最近N个报告期的财务趋势，默认 1（仅最新一期）" },
      },
      script: "stock_data.py",
      argv: (p) => {
        const args = ["financials", p.symbol];
        if (p.periods && p.periods > 1) args.push("--periods", String(p.periods));
        return args;
      },
    })
  );
  ...
  ctx.tools.register(
    pyTool({
      ...
      argv: (p) => {
        ...
        if (p.periods) args.push("--periods", p.periods);
        return args;
      },
    })
  );
  ...
  ctx.tools.register(
    pyTool({
      ...
      argv: (p) => {
        ...
        if (p.date) args.push("--date", p.date);
        if (p.symbol) args.push("--symbol", p.symbol);
        args.push("--top", String(p.top ?? 20));
        return args;
      },
    })
  );
  ...
  ctx.tools.register(
    pyTool({
      ...
      parameters: {
        top: { type: "number", description: "返回前N只，默认 20" },
      },
      script: "stock_data.py",
      argv: (p) => ["hot_stocks", "--top", String(p.top ?? 20)],
    })
  );
  ...
  ctx.tools.register(
    pyTool({
      ...
      parameters: {
        symbol: { type: "string", required: true, description: "A股股票代码，如 600519" },
        days: { type: "number", description: "返回最近N个交易日，默认 10" },
      },
      script: "stock_data.py",
      argv: (p) => ["margin_trading", p.symbol, "--days", String(p.days ?? 10)],
    })
  );
  ...
  ctx.tools.register(
    pyTool({
      ...
      script: "stock_data.py",
      argv: (p) => ["northbound_flow", "--days", String(p.days ?? 10)],
    })
  );
  ...
  ctx.tools.register(
    pyTool({
      ...
      parameters: {
        ...
        config: { type: "string", description: "自定义筛选配置YAML文件路径，不填用默认配置" },
        l2: { type: "boolean", description: "开启L2量化增强（质量/成长/真实动量/波动率/资金流因子，逐股抓取数据较慢），默认关闭" },
      },
      script: "screener.py",
      argv: (p) => {
        const args = ["screen", "--market", p.market ?? "A", "--top", String(p.top ?? 20)];
        if (p.config) args.push("--config", p.config);
        if (p.l2) args.push("--l2");
        return args;
      },
    })
  );
  ...
  ctx.tools.register(
    pyTool({
      ...
      parameters: {
        ...
        save: {
          type: "boolean",
          description: "是否归档到本地复盘 JSONL 存储，默认 false",
        },
      },
      script: "report_renderer.py",
      argv: (p) => {
        const b64 = Buffer.from(JSON.stringify(p.report), "utf-8").toString("base64");
        const args = ["market", "--input-b64", b64];
        if (p.save) args.push("--save");
        return args;
      },
    })
  );
  ...
  ctx.tools.register(
    pyTool({
      ...
      script: "market_review.py",
      argv: (p) => ["history", "--limit", String(p.limit ?? 10)],
    })
  );
    ...
    pyTool({
      ...
      argv: (p) => {
        const args = [
          ...
          p.direction,
          "--horizon-days",
          String(p.horizon_days ?? 10),
        ];
        if (p.entry_price !== undefined) args.push("--entry-price", String(p.entry_price));
        if (p.target_price !== undefined) args.push("--target-price", String(p.target_price));
        if (p.stop_price !== undefined) args.push("--stop-price", String(p.stop_price));
